...
- **2026-08-26: M2 (signature-aware candidate ranking) implemented — a real gap M2.5 explicitly left
  open, not a guess at what M2 "should" mean.** `compilers/base.py`'s own module docstring had said since
  M1 that `candidate_flags_for_signature()` kept its `signature` argument specifically so M2 could wire
  in real ranking later without an interface change — this is that fill-in. Two concrete design decisions
  worth recording:
  1. **`resource_dominance_pct` had to be threaded through for the first time** (`RunSignature` already
     carried it since M1; nothing downstream — `run_one_trial()`'s return dict, `_characterize_baseline()`,
     `BaselineResult` — ever read it). This is doc/DESIGN.md §4.3's `retiring-high-narrow-margin` row's own
     missing "margin" field, called out explicitly in `_signal_is_implausible()`'s own docstring
     ("there's no margin field to judge it by ... left to M2's own ranking") rather than guessed at
     — confirmed which field was actually the right one by checking both characterization paths
     (`wspy-archetype`'s local scorecard and the external reference-matrix corpus) for what's universally
     available: `resource_dominance_pct` is (both paths already expose it); a `confidence` string exists
     only on the reference-matrix side, so it wasn't usable as a margin proxy without a fallback gap on
     the local-characterization path.
  2. **Ranking counts *matching signals*, not just presence of any match** — the difference matters
     concretely for `-march=<uarch>` (tagged both `compute-bound` and `retiring-high-narrow-margin`)
     vs. `-Ofast`/`-ffast-math`/`-funroll-loops` (tagged `compute-bound` only). A "does *any* signal
     match" scheme would tie all four flags whenever baseline is compute-bound, never differentiating
     them — a *count* scheme gives `-march` an extra point specifically when the margin is narrow
     (`resource_dominance_pct <= _NARROW_MARGIN_MAX_PCT`, a first-cut `60.0` threshold, same
     not-yet-calibrated posture as `MIN_PRACTICAL_SIGNIFICANCE_PCT`), matching §4.3's own "diminishing
     returns for aggressive flags; `-march` for the last few percent" guidance as an actual reordering
     rather than just descriptive prose nothing ever enforced. When the margin is wide instead (a
     confidently, dominantly compute-bound shape), `-march` gets no such boost and ties with the other
     three on catalog order — deliberately not a blanket "always prefer `-march`" rule.
  `RESOURCE_DOMINANCE_SIGNALS` (the `frontend-bound`/`speculation-bound`/`compute-bound`/`backend-bound`
  vocabulary) moved from `orchestrator.py` to `compilers/base.py` in the same change, so `gcc.py`'s new
  ranking code and `orchestrator.py`'s existing filtering code share one definition instead of each
  keeping its own copy in sync by hand — a real duplication risk avoided, not just tidiness, since the
  two lists diverging silently would be exactly the kind of bug this project's own traps log exists to
  prevent for the *next* case, not just this one.
  **Verified at the mocked tier only so far** (`tests/test_compilers_gcc.py`'s dedicated ranking-fixture
  tests — every signal type, both narrow- and wide-margin compute-bound cases, an unknown-pct case, and
  an unknown-baseline case, all via a small fake `signature` object exposing only the three attributes
  the ranking code actually reads; `tests/test_orchestrator.py`'s wiring test proving
  `generate_candidates()` really passes the whole `baseline` through, not just the bare
...
| `cfm/compilers/gcc.py` | The only implementation: `candidate_flags_for_signature()` (returns the whole applicable-language catalog minus `category == "pgo"` entries — PGO is Phase 6's own dedicated two-pass flow, never an ordinary per-flag candidate, see Non-obvious traps' 2026-08-22 entry — then, since M2 (2026-08-26), genuinely reads its own `signature` arg and sorts survivors by `_candidate_rank()`: the *count* of `topdown_signals` that confidently match the baseline's shape, highest first, stable otherwise — see `_NARROW_MARGIN_MAX_PCT`/`_signal_matches()` for how this makes `-march=<uarch>` outrank other compute-bound-tagged flags specifically on a narrow-margin baseline), `validate_flagset()` (unknown-flag/conflict checks against `config/gcc_flag_catalog.seed.json`), `render_optimize_string()`, plus `benchmark_languages()` (reads a benchmark's language from SPEC's own `Spec/object.pm`). |
